...
while(1):
	print ' ' 
	nb = input('Enter (b) for RFM12B or (w) for RFM69CW>')
	#print ('Number%s \n' % (nb))
	...
	if nb==b: 
		...
		ser.close()
		print 'Attempting RFM12B 433Mhz emonTx firmware upload....'
	...
	if nb==w: 
		...
		ser.close()
		print 'Attempting RFM9CW 433Mhz emonTx firmware upload....'
...
'
		...
		if (len(linestr)>0):
			if (int(linestr[1] + linestr[2])==NodeID): 
				...
				print 'FAIL...RF received but not from the emonTx (Check DIP switch setting?)'
		else: 
