...
//  <<- end of commented out section, to save on RAM space!
  ...
  // Uncomment and upload the sketch with the following to clear EEPROM memory and reset counters
  //for (int i  = 0; i < 512; i++) eeprom_write_byte((uint8_t *)(i), 0);
...
void allGeneralProcessing()
{
  ...
  if (sampleV_minusDC_long > voltageThesholdUpper_long) {
    voltageZoneNow = POSITIVE_ZONE; }
  else
  if (sampleV_minusDC_long < voltageThesholdLower_long) {
  ...
  if (timeNow - timeOfLastPeakEntry > MAX_INTERVAL_BETWEEN_CONSECUTIVE_PEAKS)
  {
    ...
    // A couple of checks occur to check that we dont do repeat saves
    // A maximum save rate of once every 10s is allowed this is intended to stop a situation 
    // where saves may occur continuously and so use up the available writes
    
    // The laststate check, checks that the last AC voltage state was a present voltage
    // This stops a repeated eeprom save if the emontx is both powered from the AC supply and 
    // from a second supply
    if ((timeNow-lastsave)>10000 && laststate==true) {
      //digitalWrite(LEDpin, LED_ON);
      lastsave = timeNow;
      eeprom_write_block((const void*)&settings, (void*)0, sizeof(settings));
      Serial.println("Writting"); // Comment this out in production
      ...
      if (polarityOfLastSampleV != POSITIVE)
      {
        ...
        switch(sequenceCount)
        {
          ...
          case 20: 
            ...
            Serial.print(" ");
            Serial.println(settings.elapsed_time);      
            sumP_CT1 = 0;
            samplesDuringThisWindow_CT1 = 0;
            Serial.print(tx_data.realPower_CT1);
            Serial.print(':');
            Serial.print(settings.wh_CT1);
            break;
          case 40: 
          ...
          case 80: 
            ...
            Serial.print(':');
            Serial.print(settings.wh_CT4);          
           break;
          case 100: 
            tx_data.msgNumber = msgNumber++;
            tx_data.wh_CT1 = settings.wh_CT1;
            tx_data.wh_CT2 = settings.wh_CT2;
            tx_data.wh_CT3 = settings.wh_CT3;
            tx_data.wh_CT4 = settings.wh_CT4;
