...
#include "EmonLib2.h"                                                  // Include EmonLib energy monitoring library https://github.com/openenergymonitor/EmonLib
...
const int PHASE_DELAY=			  16;									// For 3-phase systems
...
// This has 2 additional variants of Power2: shifted by 120 degrees back (power2s) and by 240 degrees back (power2t)
typedef struct { int power1, Irms1, Pf1, power2, Irms2, Pf2, power3, Irms3, Pf3, power4, Irms4, Pf4, Vrms, temp, power2s, Irms2s, Pf2s,  power2t, Irms2t, Pf2t; } PayloadTX;     
...
//################################################################################################################################
  ...
  if (debug==1)
  {
    Serial.println("Amaury modifs version 1.0");
	Serial.print("Phase delay on second measure CT2: "); Serial.println(PHASE_DELAY);
  ...
  if (ACAC)
  {
    if (CT1) for (int j=0; j<5; j++) ct1.calcVI(no_of_half_wavelengths,timeout,0);
    if (CT2) for (int j=0; j<5; j++) ct2.calcVI(no_of_half_wavelengths,timeout,0);
    if (CT3) for (int j=0; j<5; j++) ct3.calcVI(no_of_half_wavelengths,timeout,0);
    if (CT4) for (int j=0; j<5; j++) ct4.calcVI(no_of_half_wavelengths,timeout,0);
   }
   else
  {
...
void loop()
{
  ...
  if (CT1) 
  {
   if (ACAC) 
   {
     ct1.calcVI(no_of_half_wavelengths,timeout, 0); emontx.power1=ct1.realPower;
     emontx.Irms1=ct1.Irms*100; // Added current
     emontx.Pf1=ct1.powerFactor*100; // Added power factor
  ...
  if (CT2) 
  {
   if (ACAC) 
   {
     ct2.calcVI(no_of_half_wavelengths,timeout, 0); emontx.power2=ct2.realPower;
     emontx.Irms2=ct2.Irms*100; // Added current
     emontx.Pf2=ct2.powerFactor*100; // Added power factor
     emontx.Vrms=ct2.Vrms*100;
	 
	 // Second measurement with phase delay of 120 degrees back
	 ct2.calcVI(no_of_half_wavelengths,timeout, PHASE_DELAY); 
	 emontx.power2s=ct2.realPower;
     emontx.Irms2s=ct2.Irms*100; // Added current
     emontx.Pf2s=ct2.powerFactor*100; // Added power factor

	 // Third measurement with phase delay of 240 degrees back
	 ct2.calcVI(no_of_half_wavelengths,timeout, PHASE_DELAY *2); 
	 emontx.power2t=ct2.realPower;
     emontx.Irms2t=ct2.Irms*100; // Added current
     emontx.Pf2t=ct2.powerFactor*100; // Added power factor
  ...
  if (CT3) 
  {
   if (ACAC) 
   {
     ct3.calcVI(no_of_half_wavelengths,timeout, 0); emontx.power3=ct3.realPower;
     emontx.Irms3=ct3.Irms*100; // Added current
     emontx.Pf3=ct3.powerFactor*100; // Added power factor
  ...
  if (CT4) 
  {
   if (ACAC) 
   {
     ct4.calcVI(no_of_half_wavelengths,timeout, 0); emontx.power4=ct4.realPower;
     emontx.Irms4=ct4.Irms*100; // Added current
     emontx.Pf4=ct4.powerFactor*100; // Added power factor
