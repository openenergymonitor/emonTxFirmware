...
void loop()
{
  ...
  if (CT1) {
  emontx.power1 = ct1.calcIrms(1480)*Vrms;   // Calculate Apparent Power 1 assuming 240Vrms AC - 1480 is  number of samples
  Serial.print(emontx.power1); 

  }
  
  if (CT2) {
  emontx.power2 = ct2.calcIrms(1480)*Vrms;   // Calculate Apparent Power 2 assuming 240Vrms AC - 1480 is  number of samples
  Serial.print(emontx.power2);  
  }
  
  if (CT3) {
  emontx.power3 = ct3.calcIrms(1480)*Vrms;   // Calculate Apparent Power 3 assuming 240Vrms AC - 1480 is  number of samples
  Serial.print(emontx.power3);  
  }
  
  if (CT4) {
  emontx.power4 = ct4.calcIrms(1480)*Vrms;   // Calculate Apparent Power 4 assuming 240Vrms AC - 1480 is  number of samples
  ...
  { 
    ...
    emontx_sleep(TIME_BETWEEN_READINGS);                                                      // sleep or delay in seconds - see emontx_lib
  }
